...
open class FullScreenSlideshowViewController: UIViewController {
    ...
    open var topStackViewFrame: CGRect?
...
//        closeButton.setImage(UIImage(named: "ic_cross_white", in: .module, compatibleWith: nil), for: UIControlState())
    ...
    func setupStackView(){
        ...
        closeButton.titleLabel?.text = "Mamun"
    ...
    open override func viewDidLayoutSubviews() {
        if !isBeingDismissed {
           ...
           // let label1 = UILabel().frame = closeButtonFrame ?? CGRect(x: max(10, safeAreaInsets.left), y: max(10, safeAreaInsets.top), width: 40, height: 40)
            stackView.frame = topStackViewFrame ?? CGRect(x: max(50, safeAreaInsets.left), y: max(15, safeAreaInsets.top), width: view.frame.width, height: 100)
        }
