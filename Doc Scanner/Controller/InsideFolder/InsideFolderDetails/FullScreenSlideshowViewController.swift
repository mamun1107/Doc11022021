...
//
//        sharebutton.setImage(UIImage(named: "ic_cross_white", in: .module, compatibleWith: nil), for: UIControlState())
//
//        sharebutton.addTarget(self, action: #selector(FullScreenSlideshowViewController.sharebuttonclicked), for: UIControlEvents.touchUpInside)
    ...
    open override func viewDidLayoutSubviews() {
        if !isBeingDismissed {
            ...
            sharebutton.frame = shareButtonFrame ?? CGRect(x: closeButton.frame.width + 10, y: max(10, safeAreaInsets.top), width: 40, height: 40)
        }
